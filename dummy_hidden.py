...
# 변조 여부 판단 시: .bin파일               ->    변조여부가 커멘드라인 메세지로 출력
#
# ex) dummy_hidden.py C:\aaa\bbb\ccc.264  C:\aaa\bbb\ddd.264    ->    ccc_ddd.264 생성
# ex) dummy_hidden.py C:\aaa\bbb\ccc.264  1                     ->    ccc_hidden.264,  ccc_hidden.yuv 생성
# ex) dummy_hidden.py C:\aaa\bbb\ccc.264                        ->    숨겨진 영상이 존재합니다.  or  숨겨진 영상이 존재하지 않습니다.
#########################################################################################################


# 더미-히든 시나리오는 더미(보여질미끼영상) 와 히든(숨길목적영상)을 하나의 스트림으로 만들어주는 시나리오 입니다
...
if goto == 1:
    ...
    stream_d, cdx  = codecdecision(stream_d)
...
ex)    dummy_hidden.py C:\aaa\bbb\ccc.264  C:\aaa\bbb\ddd.264      ->    ccc_ddd.264 생성
ex)    dummy_hidden.py C:\aaa\bbb\ccc.264  1                       ->    ccc_hidden.264,  ccc_hidden.yuv 생성
ex)    dummy_hidden.py C:\aaa\bbb\ccc.264                          ->    숨겨진 영상이 존재합니다.  or  숨겨진 영상이 존재하지 않습니다.  출력됨""")
