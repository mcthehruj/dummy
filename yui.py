...
##################
    def detect(self, text, scenario, index, name):  # 1이 반전, 2가 xor
        ...
        print('3. testing what the codec of encoded bitstream is...')  # 코덱분류
        text.insert(CURRENT, '3. testing what the codec of encoded bitstream is...\n')
        ...
        # print(frequency)
        """
        maximum = frequency[frequency.index(max(frequency))]
        max_index = frequency.index(max(frequency))
        frequency[max_index] = 0
        second_max_index = frequency.index(max(frequency))
        # print(maximum, max_index)
        if second_max_index in list(range(2, 6)):
            pass
        else:
            frequency[max_index] = maximum
        # """
        print(frequency)
        text.insert(CURRENT, 'frequency is [')
        for a in frequency:
            text.insert(CURRENT, '%d, ' %(a))
        text.insert(CURRENT, ']\n')
        print('4. testing what the scenario of encoded bitstream is...')  # 시나리오분류
        text.insert(CURRENT,'4. testing what the scenario of encoded bitstream is...\n')
        ...
        # print('5. reconstructing the encoded bitstream...')
        # text.insert(CURRENT,'5. reconstructing the encoded bitstream...\n')
        # now = datetime.now()
        # print('%d.%02d.%02d %d:%02d:%02d' % (now.year, now.month, now.day, now.hour, now.minute, now.second))
        # text.insert(CURRENT, '%d.%02d.%02d %d:%02d:%02d\n' % (now.year, now.month, now.day, now.hour, now.minute, now.second))
        # video = bitstring.BitStream('0b' + video)
...
#####
    def detect_inv(self, text, name):
        ...
        print('%d.%02d.%02d %d:%02d:%02d' % (now.year, now.month, now.day, now.hour, now.minute, now.second))
        text0 = '%d.%02d.%02d %d:%02d:%02d' % (now.year, now.month, now.day, now.hour, now.minute, now.second)
        text.insert(CURRENT, text0)
        ...
        print('5. reconstructing the encoded bitstream...')
        text.insert(CURRENT,'\n5. reconstructing the encoded bitstream...\n')
